...
QString QMPlay2CoreClass::getLongFromShortLanguage(const QString &lng)
{
    ...
    if (lng.contains(QChar('_')))
        lang += " (" + locale.nativeCountryName() + ")";
    return lang.isEmpty() ? lng : lang;
}
...
void QMPlay2CoreClass::setLanguage()
{
    ...
    const bool loaded = qtTranslator->load("qtbase_" + lang, QLibraryInfo::location(QLibraryInfo::TranslationsPath));
    Q_UNUSED(loaded)
}
